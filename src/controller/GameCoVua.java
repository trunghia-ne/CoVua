...
public class GameCoVua {
    ...
    public int trangThai(int nguoiChoi) {
        ...
        Node[] listViTri = moRongNcDi.getDanhSachMoRong();
        if (listViTri.length == 0) {
            if (kiemTraChieuTuong(nguoiChoi)) {
                trangThai = Data.DACHIEUTUONG;
            } else
                trangThai = Data.THUA;
        }
        return trangThai;
    ...
    public boolean vuaAnToan(int player, int viTriDau, int viTriCuoi) {
        ...
        Node viTriQuanCo = new Node(viTri, nuocDi);
    ...
    public boolean kiemTraChieuTuong(int player) {
        ...
        QuanCo vua = (player == Data.NGUOI) ? vuaNguoi : vuaMay;
        if (vua == null)
            return true;
        chieuTuong = totChieu(vua);
        if (!chieuTuong)
            chieuTuong = maChieu(vua);
        if (!chieuTuong)
            chieuTuong = tuongChieu(vua);
        if (!chieuTuong)
            chieuTuong = xeChieu(vua);
        if (!chieuTuong)
            chieuTuong = hauChieu(vua);
        if (!chieuTuong)
            chieuTuong = vuaChieu(vua);
        return chieuTuong;
    }

    // vì 1 hàng có 8 quân cờ và 2 ô trống 2 bên để giới hạn khung nên chỉ số ô sẽ
    // được tính như sau<==>1 hàng 10 ô
    // -21 -20 -19
//   -12  -11   -10   -9  -8
//            \  |  /
//        -1 <-  X  -> +1
...
//    +8   +9   +10  +11  +12
    ...
    public boolean totChieu(QuanCo vua) {
        ...
        if (vua == vuaNguoi) {
            ...
            if (totPhaiMay == Data.KHUNG || totTraiMay == Data.KHUNG)
                return false;
            ...
            if (totPhaiNguoi != Data.KHUNG) {
                if (totPhaiNguoi > 0 && totPhaiNguoi != Data.OTRONG
                        && viTri.cacLoaiQuanCoNguoi[totPhaiNguoi].diem == QuanCo.TOT)
                    dabiChieu = true;
            }
            if (totTraiNguoi != Data.KHUNG) {
                if (totTraiNguoi > 0 && totTraiNguoi != Data.OTRONG
    ...
    public boolean maChieu(QuanCo vua) {
        ...
        for (int viTriMa : ViTriMaChieuVua) {
            ...
            if (vua == vuaNguoi) {
                ...
                if (viTrimachieu > 0 && viTrimachieu != Data.OTRONG
    ...
    public boolean vuaChieu(QuanCo vua) {
        ...
        for (int viTriVua : viTriVuaChieuVua) {
            ...
            if (vua == vuaNguoi) {
                ...
                if (viTrivuaChieu > 0 && viTrivuaChieu != Data.OTRONG
        ...
        for (int i = 0; i < chiSoHuongDi.length; i++) {
            ...
            while (true) {
                ...
                if (vua == vuaNguoi) {
                    ...
                    if (viTriTuongChieu != Data.OTRONG)
                        break;
                } else if (vua == vuaMay) {
                    if (viTriTuongChieu > 0 && viTriTuongChieu != Data.OTRONG
                            && viTri.cacLoaiQuanCoNguoi[viTriTuongChieu].diem == QuanCo.TUONG) {
                        dabiChieu = true;
                        break;
                    } else if (viTriTuongChieu != Data.OTRONG)
                        break;
                }
            ...
            while (true) {
                ...
                if (vua == vuaNguoi) {
                    if (viTriXeChieu < 0 && viTri.cacLoaiQuanCoMay[-viTriXeChieu].diem == QuanCo.XE) {
                        dabiChieu = true;
                        break;
                    } else if (viTriXeChieu != Data.OTRONG)
                        break;
                } else if (vua == vuaMay) {
                    if (viTriXeChieu > 0 && viTriXeChieu != Data.OTRONG
            ...
            while (true) {
                ...
                if (vua == vuaNguoi) {
                    if (viTriHauChieu < 0 && viTri.cacLoaiQuanCoMay[-viTriHauChieu].diem == QuanCo.HAU) {
                        dabiChieu = true;
                        break;
                    } else if (viTriHauChieu != Data.OTRONG)
                        break;
                } else if (vua == vuaMay) {
                    if (viTriHauChieu > 0 && viTriHauChieu != Data.OTRONG
